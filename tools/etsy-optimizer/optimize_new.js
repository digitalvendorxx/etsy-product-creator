...
const listings = JSON.parse(fs.readFileSync('./listings_new_raw.json', 'utf8'));

// ─────────────────────────────────────────────
// SABİT BÖLÜMLER (her listing'de aynı)
// ─────────────────────────────────────────────
...
Available as a breathable tee for sunny days or a crewneck sweatshirt for those mornings when spring hasn't quite decided to show up yet. The DTF print is vibrant and durable — no cracking or peeling after multiple washes.`;
  ...
  else if (t.includes('hockey') || t.includes('rivalry') || t.includes('hollander') || t.includes('rozanov') || t.includes('hollanov') || t.includes('cottage') && t.includes('loser')) {
...
Makes a great end-of-year gift, new teacher present, or just a treat-yourself purchase because you definitely deserve it.`;
  ...
  else if ((t.includes('cat') || t.includes('kitten') || t.includes('meme') || t.includes('funny') || t.includes('sarcastic') || t.includes('humor')) && !(t.includes('christian') || t.includes('jesus') || t.includes('faith') || t.includes('bible') || t.includes('pray'))) {
...
Perfect for teams, events, gifts, or just because you want something that's uniquely yours. Message us before ordering if you have specific customization requests — we'll work with you to make it exactly right.`;
  ...
  // ── VINTAGE / RETRO ──
  else if ((t.includes('vintage') || t.includes('retro') || t.includes('comfort colors')) && !(t.includes('christian') || t.includes('jesus') || t.includes('faith') || t.includes('bible') || t.includes('pray'))) {
...
Perfect gift for the gamer in your life — they'll finally have something to wear that isn't the same hoodie from 2019.`;
  ...
  // ── NURSE / MEDICAL ──
  else if (t.includes('nurse') || t.includes('nursing') || /\brn\b/.test(t) || t.includes('medical') || t.includes('x-ray') || t.includes('radiol') || t.includes('rad tech') || t.includes('vet tech') || t.includes('lab tech')) {
...
A perfect gift for the Star Wars fan who already has everything — because they definitely don't have this.`;
  ...
  else if (t.includes('autism') || t.includes('neurodiv') || t.includes('different')) {
...
Available as a lightweight tee for casual days or a crewneck sweatshirt for those cozy movie marathon nights. The print is high-quality and vibrant — because even your shirt should be a little out there.`;
  ...
  // ── ANIMAL (non-cat) ──
  else if ((t.includes('raccoon') || t.includes('penguin') || t.includes('goose') || t.includes('frog') || t.includes('monkey') || t.includes('bear') || t.includes('owl')) && !(t.includes('cruise') || t.includes('travel') || t.includes('trip'))) {
...
const TAG_POOLS = {
  disney: [
    ...
    'magic kingdom souvenir', 'disney park outfit', 'disneyworld souvenir',
    'disney fan apparel', 'disney adult shirt', 'disneyland trip tee',
    'disney matching shirt', 'disney group tee', 'family vacation tee'
  ],
  easter: [
  ...
  teacher: [
    ...
    'teaching profession tee', 'teacher self care gift', 'school year shirt'
  ],
  cat: [
  ...
  autism: [
    'neurodiversity gift', 'autism awareness tee', 'special needs support',
    'autism mom gift', 'inclusion advocate tee', 'puzzle piece shirt',
    'sensory friendly gift', 'different not less tee', 'autism dad shirt',
    'neurodivergent pride', 'acceptance shirt gift', 'spectrum awareness tee'
  ],
  usa: [
...
function getCategory(title) {
  const t = (title || '').toLowerCase();
  if (t.includes('disney') || t.includes('mickey') || t.includes('minnie') || t.includes('epcot') || t.includes('disneyland')) return 'disney';
  ...
  if (t.includes('hockey') || t.includes('rivalry') || t.includes('hollander') || t.includes('rozanov')) return 'hockey';
  ...
  if (t.includes('autism') || t.includes('neurodiv') || t.includes('different')) return 'autism';
...
const optimized = listings.map(listing => {
  ...
  const newTitle = optimizeTitle2026(listing.title);
  const newTags = optimizeTags2026(optimizeTags(listing), newTitle);
...
fs.writeFileSync('./listings_new_optimized.json', JSON.stringify(optimized, null, 2), 'utf8');

console.log(`\n✅ ${optimized.length} listing optimize edildi!`);
console.log(`🏷️  Çakışan tag düzeltilen listing: ${overlapFixed}`);
console.log('📁 listings_optimized.json kaydedildi\n');
console.log('═══ ÖNİZLEME (İlk 3) ═══\n');

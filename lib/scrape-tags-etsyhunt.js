...
function scoreKeyword(kw) {
  ...
  // 6. RELEVANCE FILTERS — penalize digital/print files and non-puzzle products
  ...
  // Product is a wooden baby puzzle - block competing categories.
  const nonPuzzleProducts = ['shirt', 'tshirt', 'tee', 'hoodie', 'sweatshirt', 'sweater',
    'crewneck', 'pullover', 'dress', 'skirt', 'romper', 'pajama', 'sock',
    'towel', 'mug', 'cup', 'pillow', 'blanket', 'sticker', 'decal',
    'poster', 'canvas', 'print', 'painting', 'wall art', 'wallart',
    'hat', 'cap', 'beanie', 'apron', 'bag', 'tote', 'backpack', 'purse', 'wallet',
    'phone case', 'tumbler', 'flag', 'banner', 'rug', 'coaster', 'magnet',
    'candle', 'keychain', 'napkin', 'tablecloth',
    'pin', 'badge', 'patch', 'earring', 'necklace', 'bracelet', 'ring',
    'curtain', 'tapestry'];
  if (badTerms.some(t => lower.includes(t))) score -= 80;
  if (nonPuzzleProducts.some(t => lower.includes(t))) score -= 80;
  // Bonus for relevant puzzle context tokens
  const puzzleBoost = ['puzzle', 'wooden', 'montessori', 'name puzzle', 'baby puzzle', 'toddler', 'newborn', 'baby shower', 'first birthday'];
  if (puzzleBoost.some(t => lower.includes(t))) score += 15;
...
// Product is a WOODEN BABY NAME PUZZLE, so we vary "puzzle" / "baby puzzle" /
// "wooden puzzle" / "montessori puzzle" / "name puzzle" tails.
async function getRelatedSearchTerms(mainKeyword) {
  const PUZZLE_TAILS = ['puzzle', 'name puzzle', 'baby puzzle', 'wooden puzzle', 'montessori puzzle'];
  const tailRe = /\s+(name\s+puzzle|baby\s+puzzle|wooden\s+puzzle|montessori\s+puzzle|puzzle)$/i;
  const words = mainKeyword.toLowerCase().replace(tailRe, '').trim().split(/\s+/);
  const root = words.join(' ').trim() || mainKeyword.toLowerCase();
  const localTerms = [];
  for (const tail of PUZZLE_TAILS) localTerms.push(`${root} ${tail}`);
  if (words.length >= 2) {
    localTerms.push(`${words[0]} ${PUZZLE_TAILS[0]}`);
    localTerms.push(`${words.slice(-1)[0]} ${PUZZLE_TAILS[2]}`);
  }
  ...
  try {
    ...
    const response = await fetchWithRetry('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `I sell personalized wooden baby name puzzles on Etsy (Montessori puzzles, busy puzzles, custom letter boards for toddlers). My current design theme is: "${mainKeyword}"

Generate exactly 2 DIVERSE alternative Etsy search phrases buyers would type to find THIS SPECIFIC type of wooden baby puzzle. Each phrase MUST end with one of: "puzzle", "name puzzle", "baby puzzle", "wooden puzzle", or "montessori puzzle".

Rules:
- 2-4 words + a puzzle tail at the end
- The two phrases must use DIFFERENT lead words / different tail (e.g. one ends "name puzzle", the other "wooden puzzle")
- Must be closely related to "${mainKeyword}" theme (specific animal/object/style of design, sub-themes, occasions, aesthetics)
- What real Etsy buyers actually type
- Do NOT include shirts, tees, mugs, towels, posters, prints, decor, or unrelated products
- No brand names
...
async function scrapeEtsyHunt(mainKeyword, designContext = '') {
  ...
  // Main keyword always includes a puzzle tail
  let primaryKw = mainKeyword.trim();
  const puzzleTerms = ['puzzle'];
  if (!puzzleTerms.some(t => primaryKw.toLowerCase().includes(t))) {
    primaryKw = primaryKw + ' name puzzle';
  }
  ...
  try {
    const response = await fetchWithRetry('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `Given this Etsy personalized wooden baby name puzzle listing title: "${title}"
Design context: "${designContext}"

Extract the single BEST main keyword phrase (2-4 words) for EHunt keyword research. The keyword should describe the puzzle's theme + a puzzle tail (e.g. "farm animal name puzzle", "wooden dinosaur puzzle", "space montessori puzzle").
Rules:
- Must be what real Etsy buyers actually type
- 2-4 words maximum
- MUST include "puzzle" (preferably "name puzzle", "wooden puzzle", "baby puzzle", "montessori puzzle")
- No brand names, no apparel terms (no shirt/tee/hoodie/mug/poster)
- Most specific to the design's theme (animal/space/farm/etc.)

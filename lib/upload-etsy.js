...
// Etsy silently drops large/slow uploads. Public docs say 20MB but in practice
// >7MB PNGs frequently fail when uploaded via CDP. Always re-encode big files.
...
// Etsy silently drops photos over ~20MB. Re-encode oversized files to a JPEG
// copy under the cap and return the path that should actually be uploaded.
async function ensureUploadable(absPath) {
  try {
    ...
    console.log(`  [resize] ${path.basename(absPath)} ${(stat.size/1024/1024).toFixed(1)}MB -> ${(newSize/1024/1024).toFixed(1)}MB`);
    return outPath;
  } catch (err) {
    console.warn(`  [resize] ${path.basename(absPath)} compress failed: ${err.message} - using original`);
    return absPath;
  }
}
...
async function uploadToEtsy({ sku, mockupPaths, tags, title, description, altTexts, templateListingId, sectionId }) {
  ...
  // Listing-template photos (product-info, how-to-customize, thank-you) come from
  // assets/listing-templates/. See memory/project_post_mockup_templates.md.
  const templatesDir = path.join(__dirname, '..', 'assets', 'listing-templates');
  ...
  try {
    ...
    const TEMPLATE_LISTING_ID = templateListingId || config.templateListingId || '4486340237';
...
async function managePhotos(page, mockupPaths, templatesDir, altTexts = []) {
  ...
  // Step 2: Build full upload list. Mockups have priority over templates so
  // the photo cap never silently drops user mockups.
  // Etsy 2025+ allows up to 20 photos per listing.
  const ETSY_PHOTO_LIMIT = 20;
  const mockupAbsPaths = [];
  const missingMockups = [];
  for (const mockupPath of mockupPaths) {
    const absPath = path.resolve(mockupPath);
    if (!fs.existsSync(absPath)) {
      missingMockups.push(path.basename(absPath));
      continue;
    }
    mockupAbsPaths.push(absPath);
  }
  if (missingMockups.length > 0) {
    console.error(`  ERROR: ${missingMockups.length} mockup file(s) missing on disk: ${missingMockups.join(', ')}`);
  }
  // Listing-template photos are appended upstream by server.js (appendListingTemplates).
  // The upload path uploads whatever mockupPaths it receives verbatim, with dedup by basename
  // so the photos never duplicate if the server already added templates.
  const seenBasenames = new Set();
  const dedupedMockups = [];
  for (const p of mockupAbsPaths) {
    const key = path.basename(p);
    if (seenBasenames.has(key)) continue;
    seenBasenames.add(key);
    dedupedMockups.push(p);
  }
  const allPaths = dedupedMockups.slice(0, ETSY_PHOTO_LIMIT);
  const dropped = dedupedMockups.length - allPaths.length;
  if (dropped > 0) {
    console.warn(`  WARNING: Etsy ${ETSY_PHOTO_LIMIT}-foto siniri asildi - ${dropped} dosya atlandi.`);
  }

  console.log(`  Uploading ${allPaths.length}/${ETSY_PHOTO_LIMIT} files...`);
  ...
  // After each upload poll until the grid count goes up, otherwise the next
  // setInputFiles can hit a busy input and Etsy silently drops the file.
  for (let i = 0; i < allPaths.length; i++) {
    const uploadable = await ensureUploadable(allPaths[i]);
    const before = await page.evaluate(() => document.querySelectorAll('.le-media-grid__item img').length);
    await uploadFile(page, uploadable);
    const targetCount = before + 1;
    let appeared = false;
    for (let t = 0; t < 20; t++) {
      await page.waitForTimeout(1000);
      const cnt = await page.evaluate(() => document.querySelectorAll('.le-media-grid__item img').length);
      if (cnt >= targetCount) { appeared = true; break; }
    }
    if (!appeared) {
      console.warn(`  WARNING: ${path.basename(uploadable)} grid'e gelmedi, tekrar deniyorum...`);
      await uploadFile(page, uploadable);
      await page.waitForTimeout(3000);
    }
  }

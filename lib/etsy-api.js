...
function isConfiguredValue(value) {
  ...
  if (['placeholder', 'changeme', 'change_me', 'example', 'todo'].includes(lower)) return false;
  return true;
}
...
function getPublicEtsyApiStatus() {
  ...
  const hasApiKey = isConfiguredValue(cfg.apiKey);
  const hasAccessToken = isConfiguredValue(cfg.accessToken);
  const hasRefreshToken = isConfiguredValue(cfg.refreshToken);
  const hasUserId = isConfiguredValue(cfg.userId);
  const hasShopId = isConfiguredValue(cfg.shopId);
  const hasRedirectUri = isConfiguredValue(cfg.redirectUri);
  return {
    apiKey: hasApiKey ? mask(cfg.apiKey) : '',
    accessToken: hasAccessToken ? mask(cfg.accessToken) : '',
    refreshToken: hasRefreshToken ? mask(cfg.refreshToken) : '',
    userId: hasUserId ? mask(cfg.userId) : '',
    shopId: hasShopId ? cfg.shopId : '',
    shopName: isConfiguredValue(cfg.shopName) ? cfg.shopName : '',
    redirectUri: hasRedirectUri ? cfg.redirectUri : '',
    scopes: cfg.scopes || '',
    tokenExpiresAt: cfg.tokenExpiresAt || '',
    configured: !!(hasApiKey && hasAccessToken && hasShopId),
    oauthReady: !!(hasApiKey && hasRedirectUri),
    commercialReady: !!(hasApiKey && hasRedirectUri && hasAccessToken && hasShopId),
  };
}

function etsyHeaders({ oauth = true, contentType } = {}) {
  const cfg = getEtsyApiConfig();
  if (!isConfiguredValue(cfg.apiKey)) throw new Error('ETSY_API_KEY eksik. .env icine keystring:shared_secret olarak ekleyin.');
  if (oauth && !isConfiguredValue(cfg.accessToken)) throw new Error('ETSY_ACCESS_TOKEN eksik. OAuth token gerekli.');
...
async function listShopSections() {
  const cfg = getEtsyApiConfig();
  if (!isConfiguredValue(cfg.shopId)) throw new Error('ETSY_SHOP_ID eksik.');
...
function createOAuthStart({ redirectUri, scopes } = {}) {
  ...
  if (!isConfiguredValue(cfg.apiKey) || !clientId) throw new Error('ETSY_API_KEY keystring eksik.');
  if (!isConfiguredValue(callback)) throw new Error('ETSY_REDIRECT_URI eksik.');
...
async function exchangeOAuthCode({ code, state }) {
  ...
  if (!isConfiguredValue(cfg.apiKey) || !clientId) throw new Error('ETSY_API_KEY keystring eksik.');
  if (!isConfiguredValue(cfg.redirectUri)) throw new Error('ETSY_REDIRECT_URI eksik.');
...
async function refreshOAuthToken() {
  ...
  const clientId = getKeystring(cfg.apiKey);
  if (!isConfiguredValue(cfg.apiKey) || !clientId) throw new Error('ETSY_API_KEY keystring eksik.');
  if (!isConfiguredValue(cfg.refreshToken)) throw new Error('ETSY_REFRESH_TOKEN eksik.');
  const params = new URLSearchParams({

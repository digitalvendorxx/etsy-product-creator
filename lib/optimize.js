...
Perfect gift for the gamer in your life — they'll finally have something to wear that isn't the same hoodie from 2019.`;
  ...
  // ── NURSE / MEDICAL ──
  else if (t.includes('nurse') || t.includes('nursing') || /\brn\b/.test(t) || t.includes('medical') || t.includes('x-ray') || t.includes('radiol') || /\btech\b/.test(t)) {
...
Perfect for country music fans, rodeo lovers, state pride events, or anyone who lives that western lifestyle. Makes an amazing gift for the Texan in your life — or for yourself, because you don't need an excuse to rep your roots.`;
  }
...
// Wrap an externally-supplied intro hook (e.g. AI-generated) with the standard
// boilerplate sections so every description follows the same template.
function wrapWithTemplate(intro, title) {
  const closingOptions = [
    `Add it to your favorites so you don't lose it — and check out the rest of our shop for more designs you'll love.`,
    `Save it to your favorites and come back when you're ready. We'll be here.`,
    `Grab yours before it's gone — and don't forget to check the rest of the shop for more.`,
  ...
  return cleanIntro + AVAILABLE_STYLES + SIZING + COLORS + HOW_TO_ORDER + SHIPPING + '\n\n' + closing;
}
...
// Optional productType opt switches the filters/fallback for non-apparel
// products. When null/undefined, every behaviour matches the legacy tshirt
// pipeline byte-for-byte (single / front-back / bulk modes).
const PRODUCT_FALLBACK_TAGS = {
  wall_art:  ['wall art print', 'home decor gift', 'living room art', 'bedroom decor', 'modern wall art', 'boho wall print', 'unique art gift', 'housewarming art', 'gallery wall art', 'minimal wall art', 'gift for her', 'home gift idea', 'aesthetic decor'],
  poster:    ['art poster print', 'home decor poster', 'wall poster art', 'modern poster', 'gift for her', 'gallery print', 'apartment decor', 'minimal poster', 'housewarming gift', 'living room print', 'unique art print', 'bedroom poster', 'home gift idea'],
  mug:       ['coffee mug gift', 'ceramic mug 11oz', 'cute mug gift', 'gift for her', 'gift for him', 'birthday mug gift', 'coffee lover gift', 'tea cup gift', 'mug for friend', 'cozy mug', 'kitchen gift idea', 'office mug', 'morning coffee mug'],
  tumbler:   ['insulated tumbler', 'travel mug gift', 'gym tumbler', 'gift for her', 'gift for him', 'water bottle gift', 'mama tumbler', 'coffee tumbler', 'workout tumbler', 'custom tumbler gift', 'birthday tumbler', 'office tumbler', 'iced coffee cup'],
  sticker:   ['vinyl sticker pack', 'laptop sticker', 'water bottle sticker', 'cute sticker gift', 'aesthetic sticker', 'planner sticker', 'gift for her', 'gift for friend', 'cute decal', 'kawaii sticker', 'journal sticker', 'phone sticker', 'car decal sticker'],
  candle:    ['scented candle gift', 'soy candle gift', 'home decor candle', 'birthday candle', 'gift for her', 'housewarming candle', 'spa candle gift', 'mothers day candle', 'cozy home candle', 'wedding favor', 'self care gift', 'relaxation candle', 'aromatherapy gift'],
  blanket:   ['throw blanket gift', 'cozy fleece blanket', 'home decor throw', 'minky blanket gift', 'bed throw blanket', 'gift for her', 'baby blanket gift', 'sofa throw blanket', 'housewarming gift', 'soft throw blanket', 'modern throw', 'living room throw', 'birthday blanket'],
  pillow:    ['throw pillow cover', 'home decor pillow', 'cushion cover gift', 'sofa pillow', 'bed pillow gift', 'gift for her', 'farmhouse pillow', 'boho throw pillow', 'modern pillow', 'minimal pillow', 'living room pillow', 'housewarming gift', 'aesthetic cushion'],
  tote:      ['canvas tote bag', 'reusable shopping bag', 'cute tote gift', 'aesthetic tote', 'gift for her', 'beach tote bag', 'farmer market bag', 'book tote bag', 'large tote bag', 'birthday tote gift', 'eco friendly bag', 'work tote', 'school tote'],
  bag:       ['cute bag gift', 'gift for her', 'travel bag gift', 'minimal bag', 'aesthetic handbag', 'birthday bag', 'work bag idea', 'shoulder bag gift', 'fashion bag idea', 'unique bag', 'casual bag', 'small bag gift', 'crossbody bag idea'],
  jewelry:   ['handmade jewelry', 'gift for her', 'minimalist jewelry', 'boho jewelry gift', 'birthday gift jewelry', 'dainty necklace gift', 'aesthetic jewelry', 'unique jewelry gift', 'gold jewelry gift', 'silver jewelry gift', 'mothers day gift', 'anniversary gift', 'best friend gift'],
  keychain:  ['cute keychain gift', 'leather keychain gift', 'gift for her', 'gift for him', 'name keychain', 'birthday keychain', 'aesthetic keychain', 'car keychain', 'best friend gift', 'minimal keychain', 'small gift idea', 'stocking stuffer', 'office keychain'],
  ornament:  ['christmas ornament', 'holiday ornament gift', 'tree ornament', 'gift for her', 'family ornament gift', 'first christmas', 'birthday ornament', 'keepsake ornament', 'unique ornament gift', 'pet ornament gift', 'memorial ornament', 'wedding ornament', 'newlywed gift'],
  notebook:  ['journal notebook gift', 'aesthetic journal', 'gratitude journal', 'gift for her', 'self care journal', 'planner notebook', 'travel journal', 'lined notebook gift', 'birthday journal', 'writing journal', 'student notebook', 'minimal journal', 'cute notebook'],
  hat:       ['embroidered cap', 'baseball cap gift', 'dad hat', 'gift for him', 'gift for her', 'mama hat gift', 'trucker hat', 'birthday hat', 'unisex cap', 'casual hat', 'summer cap', 'beach hat', 'aesthetic hat'],
  hoodie:    ['cozy graphic hoodie', 'oversized sweatshirt', 'gift for her', 'unisex hoodie gift', 'birthday hoodie', 'aesthetic hoodie', 'casual hoodie', 'crewneck sweatshirt', 'graphic crewneck', 'soft hoodie', 'streetwear hoodie', 'minimalist hoodie', 'gift idea hoodie'],
  tshirt:    [], // legacy fallback applies
};

function optimizeTags(listing, opts = {}) {
  const title = listing.title || '';
  const titleWords = getTitleWords(title);
  const category = getCategory(title);
  const productType = opts.productType || null;
  const isNonApparel = productType && productType !== 'tshirt' && productType !== 'hoodie' && productType !== 'hat';
  // For non-apparel productType, swap the tee-biased pool for the product's
  // own fallback list so title-overlap replacement never injects tee tags.
  const pool = isNonApparel && PRODUCT_FALLBACK_TAGS[productType] && PRODUCT_FALLBACK_TAGS[productType].length
    ? [...PRODUCT_FALLBACK_TAGS[productType]]
    : [...(TAG_POOLS[category] || []), ...TAG_POOLS.general];

  // Hard filters — drop anything that fails these regardless of position.
  const JUNK = ['query', 'search', 'undefined', 'null', 'none', 'n/a'];
  const DIGITAL_TERMS = ['svg', 'png', 'jpg', 'jpeg', 'dxf', 'eps', 'pdf', 'ai',
    'clipart', 'sublimation', 'dtf', 'dtg', 'cricut', 'silhouette',
    'download', 'printable', 'digital', 'cut file', 'vector', 'mockup', 'mock up'];
  // Default tshirt-mode blocklist (legacy).
  const NON_SHIRT_DEFAULT = ['mug', 'cup', 'tumbler', 'pillow', 'blanket', 'sticker',
    'poster', 'canvas', 'hat', 'cap', 'apron', 'tote', 'phone case',
    'decal', 'flag', 'banner', 'rug', 'coaster', 'magnet', 'ornament',
    'candle', 'keychain', 'onesie', 'bib', 'napkin', 'topper', 'cookie',
    'invitation', 'garland', 'wreath', 'crown', 'tiara', 'patch',
    'earring', 'necklace', 'bracelet', 'ring', 'sign', 'wall art',
    'curtain', 'collar', 'leash', 'bowl', 'balloon'];
  // Apparel blocklist used when the seller selected a non-apparel productType.
  const APPAREL_BLOCK = ['shirt', 'shirts', 'tshirt', 'tshirts', 'tee', 'tees',
    'sweatshirt', 'hoodie', 'crewneck', 'sweater', 'pullover', 'apparel', 'top', 'tops'];
  const NON_SHIRT = isNonApparel ? APPAREL_BLOCK : NON_SHIRT_DEFAULT;

  function hasTerm(lower, terms) {
    return terms.some(t => {
      // word-boundary match for short tokens like "ai", "pdf"
      const re = new RegExp('(^|\\s)' + t.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&') + '(\\s|$)', 'i');
      return re.test(lower);
    });
  }

  // Sorted-words key catches reorder duplicates ("250 years freedom" == "freedom 250 years")
  const seen = new Set();

  let cleanTags = (listing.tags || []).map(t => String(t || '')).filter(tag => {
    const trimmed = tag.trim();
    if (!trimmed || trimmed.length < 3) return false;
    if (trimmed.length > 20) return false; // Etsy hard limit
    if (/^\d+$/.test(trimmed)) return false;
    const lower = trimmed.toLowerCase();
    if (JUNK.includes(lower)) return false;
    if (hasTerm(lower, DIGITAL_TERMS)) return false;
    if (hasTerm(lower, NON_SHIRT)) return false;
    // duplicate word inside tag (e.g. "shirt shirt", "freedom freedom")
    const words = lower.split(/\s+/);
    if (new Set(words).size < words.length) return false;
    // dedupe by sorted words
  ...
  // 13'e tamamla — kademeli relax: hicbir kosulda 13'in altinda donmesin.
  // For non-apparel productType, the fallback is the product's own pool so we
  // never inject "graphic tee" / "casual outfit shirt" into a wall art listing.
  const DEFAULT_FALLBACK = [
    'unique gift idea', 'trendy graphic tee', 'casual outfit shirt',
    'soft cotton apparel', 'birthday present tee', 'cool design shirt',
    'gift for friend', 'everyday wear tee', 'statement apparel',
    'minimalist tee gift', 'cozy graphic shirt', 'modern apparel gift',
    'fashion forward tee', 'present idea shirt', 'comfy graphic tee',
    'thoughtful gift tee', 'wardrobe staple tee', 'cute graphic apparel'
  ];
  const FINAL_FALLBACK = (productType && PRODUCT_FALLBACK_TAGS[productType] && PRODUCT_FALLBACK_TAGS[productType].length)
    ? PRODUCT_FALLBACK_TAGS[productType]
    : DEFAULT_FALLBACK;
  const sources = [pool, FINAL_FALLBACK];
  // Pass 1: pool + overlap constraint
  // Pass 2: pool, overlap allowed
  // Pass 3: hardcoded fallback (always returns enough fresh candidates)
  for (let pass = 0; pass < 3 && result.length < 13; pass++) {
    const allowOverlap = pass >= 1;
    const list = pass < 2 ? pool : FINAL_FALLBACK;
    for (const candidate of list) {
      if (result.length >= 13) break;
      const c = String(candidate).substring(0, 20).trim().toLowerCase();
      if (!c || used.has(c)) continue;
      if (!allowOverlap && tagOverlapsTitle(c, titleWords)) continue;
      used.add(c);
      result.push(c);
    }
  }
...
// Ensures exactly 13 valid tags. Drop-in for pipeline finalization.
// Optional productType opt switches blocklist + fallback away from tshirt
// defaults when set; legacy behaviour preserved when omitted.
function enforceThirteenTags(tags, title, productType) {
  return optimizeTags({ tags: tags || [], title: title || '' }, { productType });
}

module.exports = { generateDescription, wrapWithTemplate, optimizeTags, enforceThirteenTags, generateAltTexts, getCategory };

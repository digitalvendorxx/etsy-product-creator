...
// Human-readable product label for prompt injection (mirrors lib/ai-description.js)
const PRODUCT_TYPE_PROMPT_LABEL = {
  wall_art: 'wall art print (paper or canvas, hung on a wall)',
  mug: 'ceramic coffee mug',
  tumbler: 'insulated stainless steel tumbler',
  sticker: 'vinyl sticker',
  candle: 'scented candle in a glass jar',
  blanket: 'soft throw blanket',
  pillow: 'throw pillow with printed cover',
  tshirt: 't-shirt',
  hoodie: 'hoodie / crewneck sweatshirt',
  hat: 'hat or cap',
  bag: 'bag',
  tote: 'canvas tote bag',
  jewelry: 'handmade jewelry piece',
  keychain: 'keychain',
  ornament: 'hanging ornament',
...
async function generateLifestyleMockups({
  ...
  productDescription, productType, productDetail, sku,
  ...
  if (specs.length === 0) {
    // Explicit productType wins over keyword inference. Falls back to pickScenePool.
    let poolKey = null;
    if (productType) {
      const preset = THEME_PRESETS[productType];
      if (preset && preset.pool && SCENE_POOLS[preset.pool]) poolKey = preset.pool;
      else poolKey = pickScenePool(productType + ' ' + (productDetail || ''));
    }
    if (!poolKey) {
      const hint = [(productDescription || ''), ...(themeWords || [])].join(' ');
      poolKey = pickScenePool(hint);
    }
  ...
  for (let i = 0; i < total; i++) {
    ...
    const typeLabel = productType ? (PRODUCT_TYPE_PROMPT_LABEL[productType] || productType) : null;
    const typePrefix = typeLabel ? `This product is a ${typeLabel}. ` : '';
    const detailSuffix = productDetail ? ` Additional seller-provided detail: ${productDetail}.` : '';
    const baseDesc = productDescription || 'product';
    const productDescForPrompt = useTextOnly
      ? `${typePrefix}${baseDesc}. Visual details: ${visualDescription}.${detailSuffix}`
      : `${typePrefix}${baseDesc}.${detailSuffix}`;

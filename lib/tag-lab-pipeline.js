...
function pickBest(allRows, want = 13, seedKeywords = [], themeWords = []) {
  ...
  for (const r of allRows) {
    ...
    if (BLOCKED_PRODUCTS.test(k)) continue;
...
async function runTagLabPipeline(opts) {
  ...
  let { keywords, themeWords } = await extractKeywords(imageBuffer, mime, apiKey);
  ...
  for (let pass = 0; pass <= maxRetries; pass++) {
    ...
    best = pickBest(allRows, targetCount, allSeeds, allThemeWords);
    ...
    try {
      const more = await extractKeywords(imageBuffer, mime, apiKey);
  ...
  if (best.length) {
    ...
    try {
      ...
      const analysis = await analyzeMockup(tmpPath, { tags: best.map(r => r.keyword), apiKey });
